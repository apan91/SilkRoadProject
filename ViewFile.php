...
<div id="container">
  ...
  <ul class="dropdown-menu">
    <li><a tabindex="-1" href="ViewFile.php?sort=Oldest">Oldest</a></li>
    <li><a tabindex="-1" href="ViewFile.php?sort=MostRecent">Most Recent</a></li>
    <li><a tabindex="-1" href="ViewFile.php?sort=User">User</a></li>
    <li><a tabindex="-1" href="ViewFile.php?sort=FileName">File Name</a></li>
    <li><a tabindex="-1" href="ViewFile.php?sort=Type">File Type</a></li>
  </ul>
</div>
			
			<table class="table table-hover" cellpadding="5" cellspacing="5">
			<tr><th>#</th><th>File Name</th><th>Type</th><th>Uploaded Date</th><th>Caption</th><th>Uploader</th><th>Download</th><th>Delete</th></tr>
			<?php
...
//			else {
			...
			$file = $mysqli->query($query);
				...
				while($result = $file->fetch_assoc()) {
					...
					if($result['User_ID'] == $userid || $userperm == 4) {
						if(isset($_GET['sort']) && !empty($_GET['sort'])) {
							print("<a href='ViewFile.php?sort=".$_GET['sort']."&delete=".$result['File_ID']."'>Delete</a>");
						}
						else
